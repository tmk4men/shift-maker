...
/* =======================================================================
   シナリオ6c：契約の最低日数・最低時間の保障
   ======================================================================= */
T('シナリオ6c：契約で保障した最低日数・最低時間を満たす');
{
  const d = baseData();
  fillAvail(d, 2026, 8, 'all');
  const e4 = d.employees.find(e => e.id === 'e4');
  e4.minDays = 12; e4.minHoursMonth = 100;      // 月12日・100時間を保障
  const { data, res } = run(d);
  const s = res.stats.e4;
  console.log(`  高橋: ${s.days}日 / ${s.hours}時間（契約 12日・100時間）`);
  ok(audit(data, res.assignments).length === 0, 'ハード制約の違反なし');
  ok(s.days >= 12, '最低出勤日数を満たす', `${s.days}日`);
  ok(s.hours >= 100, '最低勤務時間を満たす', `${s.hours}時間`);

  // 満たせない場合は不足として報告されること
  const d2 = baseData();
  fillAvail(d2, 2026, 8, 'all');
  const x = d2.employees.find(e => e.id === 'e8');
  x.minHoursMonth = 300;                         // 明らかに無理な保障
  const r2 = run(d2).res;
  const v = r2.violations.filter(vv => vv.ruleId === 'OPS-034' && vv.empId === 'e8');
  ok(v.length > 0, '満たせない場合は不足として報告する', JSON.stringify(v.map(z => z.msg)));
  ok(v.some(z => z.msg.indexOf('時間不足') >= 0), '不足時間が数字で出る', v.map(z => z.msg).join(' / '));
}

/* =======================================================================
   シナリオ6d：お店の休みの日
   ======================================================================= */
T('シナリオ6d：定休日・臨時休業日には誰も入れない');
{
  const d = baseData();
  fillAvail(d, 2026, 8, 'all');
  d.settings.closedWeekdays = [2];                       // 毎週火曜が定休日
  d.settings.closedDates = ['2026-08-13', '2026-08-14']; // お盆に臨時休業
  const { data, res } = run(d);
  const dates = U.monthDates(2026, 8);

  const tuesdays = dates.filter(x => U.weekdayOf(x) === 2);
  const workedOnClosed = dates.filter(x => Store.isClosed(x))
    .filter(x => data.shiftTypes.some(st => ((res.assignments[x] || {})[st.id] || []).length > 0));
  console.log(`  休業日 ${dates.filter(x => Store.isClosed(x)).length}日（火曜${tuesdays.length}日＋臨時2日）`);
  ok(workedOnClosed.length === 0, '休業日に勤務が1件も入らない', workedOnClosed.join(','));
  ok(res.violations.filter(v => v.ruleId === 'OPS-A09').length === 0, '休業日の違反が出ない');
  ok(audit(data, res.assignments).length === 0, 'ハード制約の違反なし');

  // 営業日はちゃんと埋まる
  const openDates = dates.filter(x => !Store.isClosed(x));
  let need = 0, got = 0;
  openDates.forEach(x => data.shiftTypes.forEach(st => {
    need += Store.needOf(x, st.id);
    got += Math.min(Store.needOf(x, st.id), ((res.assignments[x] || {})[st.id] || []).length);
  }));
  ok(need > 0 && got / need > 0.95, '営業日は充足する', `${(got / need * 100).toFixed(1)}%`);
  ...
  a[cd] = { A: ['e1'] };
  const rv = Solver.revalidate(data, a);
  ok(rv.violations.some(v => v.ruleId === 'OPS-A09'), '休業日への手動割当を検出する');
}
