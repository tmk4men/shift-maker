...
function bootStaffMode(query, prepare) {
  const sb = {
    ...
    localStorage, sessionStorage: makeSession(), alert: m => alerted.push(String(m)), confirm: () => true,
    Blob: function () { }, URL: { createObjectURL: () => 'blob:x', revokeObjectURL() { } },
    FileReader: function () { this.readAsText = () => { }; },
...
tryRun('?staff=e1 で提出画面だけが開く', () => {
  staffBoot = bootStaffMode('?staff=e1', sb => { sb.Store.loadDemo(); });
});
if (staffBoot) {
...
tryRun('シフト未作成なら希望提出画面から始まる', () => {
  const b = bootStaffMode('?staff=e1', sb => { sb.Store.loadDemo(); });
  const txt = b.ids['panel-request'].all().map(n => n._text || '').join(' ');
  if (txt.indexOf('提出する') < 0) throw new Error('提出画面が出ていない');
});

tryRun('初期状態は空、サンプルは任意で読み込める', () => {
  delete store['shift-maker-v1'];
  const b = bootStaffMode('', sb => { });
  const S = b.sb.Store;
  if (S.get().employees.length !== 0) throw new Error('初期状態が空でない');
  if (!S.get().shiftTypes.length) throw new Error('勤務区分の雛形がない');
  S.loadDemo();
  if (!S.isSample()) throw new Error('サンプルを読み込めていない');
  S.startFresh();
  const d = S.get();
...
tryRun('存在しないスタッフIDでも落ちない', () => {
  const b = bootStaffMode('?staff=zzz', sb => { sb.Store.loadDemo(); });
  if (b.ids['panel-request'].children.length === 0) throw new Error('案内が出ない');
});

tryRun('スタッフ入力ページ（?input=1）が単体で開ける', () => {
  const b = bootStaffMode('?input=1', sb => { sb.Store.loadDemo(); });
...
tryRun('入力ページ → ファイル相当のデータ → 店長が取り込む', () => {
  ...
  const a = bootStaffMode('?input=1', sb => { sb.Store.loadDemo(); });
...
tryRun('画面操作での提出コード往復（SHIFT1: の encode → decode）', () => {
  ...
  const a = bootStaffMode('?staff=e1', sb => { sb.Store.loadDemo(); });
  const S = a.sb.Store;
  ...
  const b = bootStaffMode('', sb => { sb.Store.loadDemo(); });
...
/* ---------- 管理画面のロック ---------- */
console.log('\n=== 管理画面のロック ===');
tryRun('パスワードを設定すると管理タブが開かなくなる', () => {
  delete store['shift-maker-v1'];
  const b = bootStaffMode('', sb => { sb.Store.loadDemo(); sb.Store.setPin('1234'); sb.Store.lockNow(); });
  const S = b.sb.Store;
  if (!S.hasPin()) throw new Error('PINが設定されていない');
  if (S.isUnlocked()) throw new Error('ロックされていない');

  // 設定タブを開こうとするとPIN入力が出る
  const tabs = b.sb.document.querySelectorAll('.tab');
  const t = tabs.find(x => x.dataset.tab === 'setup');
  t.classList.contains = c => c === 'tab';
  (b.ids['tabs']._listeners.click || []).forEach(f => f({ target: t }));
  const txt = b.ids['modalBody'].all().map(n => n._text || '').join(' ');
  if (txt.indexOf('パスワード') < 0) throw new Error('パスワード入力が出ない');
  if (b.ids['panel-setup'].children.length) throw new Error('設定画面が描画されてしまっている');
});

tryRun('正しいパスワードで解除でき、間違いでは解除されない', () => {
  delete store['shift-maker-v1'];
  const b = bootStaffMode('', sb => { sb.Store.loadDemo(); sb.Store.setPin('4321'); sb.Store.lockNow(); });
  const S = b.sb.Store;
  if (S.tryUnlock('0000')) throw new Error('間違ったパスワードで解除できてしまう');
  if (S.isUnlocked()) throw new Error('失敗後に解除状態になっている');
  if (!S.tryUnlock('4321')) throw new Error('正しいパスワードで解除できない');
  if (!S.isUnlocked()) throw new Error('解除状態にならない');
  S.lockNow();
  if (S.isUnlocked()) throw new Error('再ロックできない');
});

tryRun('パスワードは平文で保存されない', () => {
  delete store['shift-maker-v1'];
  const b = bootStaffMode('', sb => { sb.Store.loadDemo(); sb.Store.setPin('9876'); });
  const raw = store['shift-maker-v1'] || '';
  if (raw.indexOf('9876') >= 0) throw new Error('パスワードがそのまま保存されている');
  // 英数字のパスワードも使えること
  S2check(b.sb.Store);
  function S2check(S) {
    S.setPin('sfift00');
    if (!S.tryUnlock('sfift00')) throw new Error('英数字のパスワードで解除できない');
    if (S.tryUnlock('shift00')) throw new Error('別の文字列で解除できてしまう');
    if ((store['shift-maker-v1'] || '').indexOf('sfift00') >= 0) throw new Error('英数字パスワードが平文で保存されている');
  }
  if (!b.sb.Store.get().settings.pinHash) throw new Error('ハッシュが保存されていない');
});

tryRun('スタッフ用の画面はパスワードがあっても開ける', () => {
  delete store['shift-maker-v1'];
  const b = bootStaffMode('?input=1', sb => { sb.Store.loadDemo(); sb.Store.setPin('1111'); sb.Store.lockNow(); });
  if (b.ids['panel-request'].children.length === 0) throw new Error('入力ページが開けない');
  const c = bootStaffMode('?staff=e1', sb => { sb.Store.loadDemo(); sb.Store.setPin('1111'); sb.Store.lockNow(); });
  if (c.ids['panel-request'].children.length === 0) throw new Error('スタッフ画面が開けない');
});

tryRun('パスワードを解除すれば元どおり開ける', () => {
  delete store['shift-maker-v1'];
  const b = bootStaffMode('', sb => { sb.Store.loadDemo(); sb.Store.setPin('2468'); sb.Store.lockNow(); });
  const S = b.sb.Store;
  S.setPin('');
  if (S.hasPin()) throw new Error('解除できていない');
  if (!S.isUnlocked()) throw new Error('ロックが残っている');
});
